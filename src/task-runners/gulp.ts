...
export async function gulpInitViews<T>(
	...
	return Through.obj(function(file: Vinyl, _:any, cb: Through.TransformCallback){
		...
		try{
			// Exclude streams
			if(file.isStream()) throw new Error(`Streams are not supported. Input file: ${file.path}`);
			// Get content
			var content= file.isBuffer()? file.contents.toString('utf-8') : readFileSync(file.path, 'utf-8');
			// Compile content
...
function _resolveI18n(globPattern: string, i18nVarname:string){
	...
	for(let i=0; i<len; ++i){
		...
		let m= require(file)?.[i18nVarname];
		let locale= m?.locale;
		if(typeof locale !== 'string')
			throw new Error(`Missing "${i18nVarname}.locale" inside file: ${file}`);
		if(i18nMap.has(locale))
			throw new Error(`Duplicated locale "${locale}". Last one in: ${file}`);
